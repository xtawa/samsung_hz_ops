...
- The selected direction is an authentic Google/Pixel Material 3 settings surface.
- The home screen is intentionally limited to core refresh-rate controls: live Hz, display mode, minimum/maximum range, power-saving high refresh, and permission status.
- The complete prototype is organized into four top-level destinations: Control, Rules, Tools, and More. Secondary capabilities use flat grouped settings lists and focused detail pages; they do not expand the home screen.
- Avoid dashboards, gauges, gradients, decorative illustrations, feature grids, and card-heavy layouts. A Material 3 bottom navigation bar is the approved app-wide navigation for the four destinations.
- This branch is a UI-only prototype. Controls use local mock state and must not call Android settings, Shizuku, ADB, root, or device APIs.
